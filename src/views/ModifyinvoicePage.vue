    ...
    <ion-content :fullscreen="true">
      <div class="factures-header">
        <div class="header-row">
          <ion-label class="title">Edit invoice : #</ion-label>
        </div>
      ...
      <div class="content-wrapper">
        <ion-accordion-group class="all">         
          <ion-accordion value="invoiceDetails">
            ...
            <div slot="content" class="accordion-content">
              ...
              <ion-item>
                ...
                <div class="in-cadre">
                  <ion-input class="in-cadre-input" :value="currentYear"></ion-input>
                  <ion-input class="side" :value="invoiceForm.invoice_number"></ion-input>
                </div>
              </ion-item>             
              <ion-item>
              ...
              <ion-item>
                ...
                <ion-input invoiceForm.referenceNumber class="in-cadre" placeholder="Enter reference number"></ion-input>
              </ion-item>
              ...
              <ion-text color="danger" v-if="generalTermsError">
                {{ generalTermsError }}
              </ion-text>
              ...
              <ion-item>
                <div class="add-files">
                  ...
                  <div class="attachments-list">
                    <div
                        v-for="(attachment, index) in invoiceForm.attachments"
                        :key="index"
                        class="attachment-item">
                        <ion-icon name="trash-outline" class="trash-icon" @click="removeAttachment(index)"></ion-icon>
              ...
              <div class="totals">
                ...
                <ion-item class="custom-item" v-for="item in invoiceForm.items" :key="item.id">
                  <div class="row" v-if="item.taxes && item.taxes.length">
                    <div v-for="tax in item.taxes" :key="tax.hashed_id" class="row">
                      <ion-label class="custom-label">
                        <strong>{{ tax.title }}</strong> ({{ tax.rate }}%)
                      </ion-label>
                      <div class="custom-input-wrapper">
                        <ion-input class="custom-content">
          ...
          <ion-accordion value="content">
            ...
            <div slot="content" class="accordion-content">
              ...
              <ion-item>
                ...
                <ion-item v-show="invoiceForm.show_bank === 1">
          ...
          <ion-accordion value="withholding-tax">
              <ion-item class="aTitle" slot="header">
              ...
              <div slot="content" class="accordion-content">
                <ion-item>
                  <ion-select v-model="invoiceForm.withholding_type_id" class="select-input-modal">
                    <ion-select-option 
                      v-for="w in withholdings" 
                      :key="w.hashed_id" 
                      :value="w.hashed_id">
                      {{ w.formatted_title }}
                    </ion-select-option>
            ...
            <div slot="content" class="accordion-content">
              <ion-item lines="none" class="toggle-row">
                <div class="a-toggle">
                  ...
                  @update:modelValue="(val) => invoiceForm.recurrence = val ? 2 : 1"></ion-toggle>
                </div>
              </ion-item>
              <div v-if="invoiceForm.recurrence === 2" class="recurrence-inputs">
                ...
                <ion-item>
                  ...
                  <ion-select class="select-input" v-model="invoiceForm.recurrence_type">
                    <ion-select-option :value="1">Day(s)</ion-select-option>
                    <ion-select-option :value="2">Week</ion-select-option>
                    <ion-select-option :value="3">Month</ion-select-option>
                    <ion-select-option :value="4">Year</ion-select-option>
                  </ion-select>
                </ion-item>
              </div>
            </div>
          ...
          <ion-accordion value="remarque">
            ...
            <div slot="content" class="accordion-content">
              <ion-textarea placeholder="Enter any notes" v-model="invoiceForm.remarks"></ion-textarea>
            </div>
          </ion-accordion>
        </ion-accordion-group>
      ...
      <ion-modal :is-open="isArticleDetailsModalOpen" @didDismiss="closeArticleDetailsModal" class="article-model">
        ...
        <ion-content class="ion-padding" v-if="selectedArticle">
          ...
          <ion-item class="tax-item">
            ...
            <div class="tax-container" v-for="(tax, index) in (selectedArticle.taxes.length > 0 ? selectedArticle.taxes : [{}])" :key="index">
              <ion-select
                class="tax-select"
                :value="tax.id"
                @update:modelValue="onTaxChange($event, selectedArticle.id, index)">
                <ion-select-option 
                  v-for="t in taxes" 
                  :key="t.hashed_id" 
                  :value="t.hashed_id">
              ...
              <ion-button
                class="add-tax-btn"
                @click="addTax(selectedArticle.id)">+</ion-button>
            </div>
          </ion-item>
          <ion-item>
          ...
          <ion-item>
            <ion-label>Linked to the accounting journal</ion-label>
            <ion-select class="select-input" v-model="selectedJournalId">
              <ion-select-option 
...
import { 
  ...
  IonButton, IonButtons, IonIcon, IonModal, IonInput, IonSelectOption, IonSelect,
  IonTextarea, IonRadioGroup, IonRadio, IonText, IonToggle , IonCol, IonRow, IonGrid
} from '@ionic/vue';
import { addIcons } from 'ionicons';
import { 
  calendarOutline, chatbubbleOutline ,trashOutline, closeOutline, location, addOutline, checkmarkCircleOutline,  chevronBackOutline, chevronForwardOutline, createOutline, refreshOutline
} from 'ionicons/icons';
import { computed } from 'vue';
import { useModifyInvoiceController } from '@/controllers/ModifyinvoiceController';
...
const {
    ...
    validateAndSubmit,
    saveArticle
} = useModifyInvoiceController();

const generalTermsError = computed(() => {
  return invoiceForm.generalTerms.trim().length > 0 && invoiceForm.generalTerms.trim().length < 5
    ? 'Conditions must be at least 5 characters long.'
    : '';
});
