<template>
    <ion-page>
      ...
      <ion-content class="ion-padding">
        ...
        <div class="action-container">
          <ion-searchbar
            v-model="searchQuery"
            placeholder="Rechercher utilisateurs ou rôles..."
                ...
                <ion-item class="custom-item">
                  ...
                  <ion-label>
                    ...
                    <div class="user-meta">
                      ...
                      <span v-if="user.status" class="status-indicator active">Actif</span>
                      <span v-else class="status-indicator inactive">Inactif</span>
                    </div>
                  </ion-label>
      ...
      <ion-modal :is-open="showAddUserModal" @didDismiss="showAddUserModal = false">
        ...
        <ion-content class="ion-padding">
          <ion-item class="form-item">
            <ion-label position="stacked">Nom Complet <span class="required">*</span></ion-label>
          ...
          <ion-item class="form-item">
            <ion-label position="stacked">Rôle <span class="required">*</span></ion-label>
          ...
          >
            ...
            {{ isInvitingUser ? 'Envoi en cours...' : 'Ajouter' }}
          </ion-button>
        </ion-content>
      </ion-modal>
      ...
      <ion-modal :is-open="showRoleModal" @didDismiss="closeRoleModal">
        <ion-header>
          <ion-toolbar>
            <ion-title>{{ isEditMode ? 'Modifier le Rôle' : 'Créer un Nouveau Rôle' }}</ion-title>
            <ion-buttons slot="end">
        ...
        <ion-content class="ion-padding">
          ...
          <ion-item class="form-item">
            <ion-label position="stacked">Titre du Rôle <span class="required">*</span></ion-label>
          ...
          <div class="permissions-section">
            <ion-list-header class="section-header">
              <ion-label>
                <h2>Permissions</h2>
                <p class="permissions-count">{{ selectedPermissionsCount }} permission(s) sélectionnée(s)</p>
              </ion-label>
            </ion-list-header>
          ...
          >
            ...
            {{ isLoading ? (isEditMode ? 'Modification en cours...' : 'Création en cours...') : (isEditMode ? 'Modifier le Rôle' : 'Créer le Rôle') }}
          </ion-button>
        </ion-content>
      </ion-modal>
      ...
      <ion-alert
        ...
        header="Confirmer la suppression"
        :message="`Êtes-vous sûr de vouloir supprimer le rôle ${roleToDelete?.title} ?`"
      ...
      <ion-alert
        ...
        :message="`Êtes-vous sûr de vouloir révoquer l'accès de ${userToRevoke?.name} ?`"
  ...
  const getActionClass = (permissionName: string): string => {
    ...
    switch(action) {
      case 'créer': return 'action-create';
      case 'accès': return 'action-access';
      case 'mise à jour': return 'action-update';
      case 'supprimer': return 'action-delete';
      default: return '';
    }
  };
  ...
  const handleRevokeAccess = async () => {
    try {
      if (userToRevoke.value) {
        ...
        await showToast('Accès révoqué avec succès', 'success');
      }
    } catch (error) {
      console.error('Erreur lors de la révocation:', error);
      await showToast(error?.message || 'Erreur lors de la révocation', 'danger');
    }
  };
  ...
  const handleAddUser = async () => {
    try {
      if (!newUser.fullName || !newUser.email || !newUser.roleId) {
        await showToast('Tous les champs sont obligatoires', 'danger');
        return;
      }
      ...
      if (response) {
        await showToast('Utilisateur invité avec succès', 'success');
      ...
      await showToast(error?.message || 'Erreur lors de l\'invitation', 'danger');
    }
  };
  ...
  const openEditRoleModal = async (role: any) => {
    try {
      ...
      await showToast('Erreur lors du chargement des détails du rôle', 'danger');
    }
  };
  ...
  const handleSubmitRole = async () => {
    try {
      if (!roleForm.title.trim()) {
        await showToast('Le titre du rôle est requis', 'danger');
        return;
      }
  
      if (selectedPermissions.value.size === 0) {
        await showToast('Au moins une permission doit être sélectionnée', 'danger');
        return;
      }
      ...
      if (isEditMode.value && currentEditingRole.value) {
        ...
        await showToast('Rôle modifié avec succès');
      } else {
        await createNewRole(roleData);
        await showToast('Rôle créé avec succès');
      }
      ...
      const errorMessage = error?.message || 'Erreur lors de la sauvegarde du rôle';
  ...
  const handleDeleteRole = async (roleId: string) => {
    try {
      ...
      await showToast(error?.message || 'Erreur lors de la suppression du rôle', 'danger');
    }
  };
